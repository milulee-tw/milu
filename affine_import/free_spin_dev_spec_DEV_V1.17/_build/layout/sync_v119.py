#!/usr/bin/env python3
"""DEV V1.19 → AFFiNE 一鍵同步（內網通了之後直接跑這支）。
...
NEW_IMAGES = ["fs_hist_01_activity_tab.png", "fs_hist_02_fs_card_list.png"]
HUBS = ["00", "01", "02", "03", "04", "05", "09"]
OLD_BASIS = ("依據：規格 DEV V1.18（新增結算狀態機、注單機制與遊戲端錯誤訊息；"
             "其餘產品規則沿用 DEV V1.15）")
NEW_BASIS = ("依據：規格 DEV V1.19（上限欄位統一為 max_bonus；新增結算狀態機、注單機制與玩家端錯誤文案；"
             "其餘產品規則沿用 DEV V1.15）")
        ...
        for code, doc in ids.items():
            ...
            for b in live["blocks"]:
                ...
                if f in tot:
                    tot[f] += 1
                t = b.get("text") or ""
                if "規格 DEV V1.17" in t or "規格 DEV V1.18" in t or "max_conversion" in t and "原稱" not in t and "原本使用" not in t:
                    old_ver += 1
